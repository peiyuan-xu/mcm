...
demand=[0;demand];
[m, n ,D ,C]=input_C(aplot,demand,33,8);
%n=33   病区为32 个
Time=D./5.4; %33*33
...
%清零
 ...
 for j=2:n
     for i=1:m   %选定  第i 只蚂蚁 外层 j 次循环  要去掉一个点
        ...
        %PathEndTime(i,1)=TimeEndTi(i,1)+Time(visited(j-1),to_visit);
         ...
         endnode=visited(j-1);
         [to_visitFlag,to]=panDuanCity(n,J,endnode,to_visit,Time,TimeEnd,PathEndTimeI,Eta,Tau,visited);  %判断能否选择下一城市 ，能返回 1 否则 返回 0
         ...
         if to_visitFlag==1  
              ...
              if CarFlag==1
                  ...
                  %返回 下一路径开始点 和上一路径结束点
                  TabuI=Tabu(i,:);
                  [ToNew,PathEnd]=huiYuanDian(J,visited,Eta,Tau,TabuI);
 ...
 [n_bestRoad L_bestRoad ave road_L]=select_bestRoad(PathTiaoEndNode(i,:),Tabu,D,m,n);
 ...
 %%第六步，禁忌表清零，为下次迭代做准备
  disp(['第',num2str(NC),'次迭代']);
   NC=NC+1;
   disp(['当次迭代中一只蚂蚁的路线：',num2str(Tabu(1,:))]);
   disp(['每条路径的末尾顶点：',num2str(PathTiaoEndNode(1,:))]);
  %清零
   Tabu=zeros(m,n);
   PathEndTime(:,1)=0;
  PathTiaoEndNode(:,:)=0;
...
output_result(L_best,R_best,C,R_best);
