function [to_visitFlag,to] = panDuanCity(n,J,endnode,to_visit,Time,TimeEnd,PathEndTime,Eta,Tau,visited)
...
if TimeEnd(to_visit,1)>lastTime   %时间宽裕
    ...
    for i=1:jn   %遍历个 jn 个剩余的点，看能否找到适合的
         jn=1;
         for k=1:n   
             if length(find(NoNode==k))==0
                 NoNode(jn)=k;
                 jn=jn+1;
             end
         end
       zi_to_visit=select_nextCity(J,visited,Eta,Tau);  
